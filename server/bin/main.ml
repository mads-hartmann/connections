...
let run db_path port no_scheduler log_file =
  ...
  (* Set handler contexts for feed refresh, OPML import, URL metadata, URI refresh, and connection refresh *)
  Handlers.Rss_feed.set_context ~sw ~env;
  Handlers.Import.set_context ~sw ~env;
  Handlers.Url_metadata.set_context ~sw ~env;
  Handlers.Uri_handler.set_context ~sw ~env;
  Handlers.Connection.set_context ~sw ~env;
  (* Start background jobs unless disabled *)
  if not no_scheduler then (
    Cron.Feed_sync.start ~sw ~env;
    Cron.Uri_metadata_sync.start ~sw ~env);
  Log.info (fun m ->
