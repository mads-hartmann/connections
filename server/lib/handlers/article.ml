...
let list_by_feed request =
  ...
  | Ok feed_id -> (
      ...
      | Ok (Some _) -> (
          ...
          | Error msg -> Lwt.return (Response.internal_error msg)
          | Ok paginated ->
              Lwt.return (Response.json_response (Model.Article.paginated_to_json paginated))))
...
let mark_read request =
  ...
  | Error msg -> Lwt.return (Response.bad_request msg)
  | Ok id -> (
      let* parsed = Response.parse_json_body Model.Article.mark_read_request_of_yojson request in
      match parsed with
      | Error msg -> Lwt.return (Response.bad_request msg)
      | Ok { read } -> (
