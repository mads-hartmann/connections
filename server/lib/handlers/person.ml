...
let update request id =
  ...
  if String.trim name = "" then Handler_utils.bad_request "Name cannot be empty"
    ...
    let* person =
      Service.Person.update ~id ~name ~photo |> Handler_utils.or_person_error
    in
    Handler_utils.json_response (Model.Person.to_json person)
...
(* Find the Website metadata field for a person *)
let find_website_url (person : Model.Person.t) =
  List.find_opt
    ...
    (Model.Person.metadata person)
  |> Option.map Model.Person_metadata.value

(* Refresh metadata preview - fetches metadata from person's website and returns proposed changes *)
let refresh_metadata_preview _request id =
  let* person = Service.Person.get ~id |> Handler_utils.or_person_error in
  let* website_url =
    find_website_url person
    |> Handler_utils.or_bad_request_opt
         "Person has no Website metadata field. Add a website URL first."
  in
  let* valid_url =
  ...
  let current_metadata = Model.Person.metadata person in
  ...
  let current_metadata_json =
    List.map Model.Person_metadata.to_json current_metadata
  in
  let response =
    `Assoc
      ([
      ...
      @ [
          ...
          ("current_name", `String (Model.Person.name person));
        ]
      @ opt_field "current_photo" (Model.Person.photo person)
...
let routes () =
  ...
  [
    ...
    get (s "persons" / int / s "refresh-metadata")
