...
let build () =
  ...
  App.routes ~not_found
    [
      ...
      post (s "persons" / int64 / s "feeds") |> request
      |> into Handlers.Rss_feed.create;
      get (s "persons" / int64 / s "feeds") |> request
      |> into Handlers.Rss_feed.list_by_person;
      (* Person-Category routes *)
      get (s "persons" / int64 / s "categories") |> request
      |> into Handlers.Category.list_by_person;
      post (s "persons" / int64 / s "categories" / int64) |> request
      |> into Handlers.Category.add_to_person;
      delete (s "persons" / int64 / s "categories" / int64) |> request
      ...
      post (s "feeds" / int64 / s "refresh") |> request
      |> into Handlers.Rss_feed.refresh;
      (* Article routes - nested under feeds *)
      get (s "feeds" / int64 / s "articles") |> request
      |> into Handlers.Article.list_by_feed;
      post (s "feeds" / int64 / s "articles" / s "mark-all-read") |> request
      ...
      post (s "articles" / int64 / s "read") |> request
      ...
      delete (s "categories" / int64) |> request
      |> into Handlers.Category.delete;
      (* Import routes *)
      post (s "import" / s "opml" / s "preview") |> request
      |> into Handlers.Import.preview;
      post (s "import" / s "opml" / s "confirm") |> request
