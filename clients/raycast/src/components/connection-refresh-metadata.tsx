...
interface ConnectionRefreshMetadataProps {
  connection: Connection.Connection;
  revalidate: () => void;
}

export function ConnectionRefreshMetadata({ connection, revalidate }: ConnectionRefreshMetadataProps) {
  const { pop } = useNavigation();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const { isLoading, data: preview, error } = usePromise(() => Connection.fetchRefreshMetadataPreview(connection.id));
  ...
  async function handleSubmit(values: Record<string, unknown>) {
    ...
    try {
      ...
      if (updateName && preview.proposed_name && preview.proposed_name !== preview.current_name) {
        updates.push(
          Connection.updateConnection(
            ...
            updatePhoto && preview.proposed_photo ? preview.proposed_photo : connection.photo,
          ),
        );
      } else if (updatePhoto && preview.proposed_photo && preview.proposed_photo !== preview.current_photo) {
        updates.push(Connection.updateConnection(connection.id, connection.name, preview.proposed_photo));
      }
      ...
      for (const feed of preview.proposed_feeds) {
        ...
        if (values[key] === true) {
          updates.push(Feed.createFeed(connection.id, feed.url, feed.title || ""));
        }
      }
      ...
      for (const profile of preview.proposed_profiles) {
        ...
        if (values[key] === true && !existingUrls.has(profile.url)) {
          updates.push(Connection.createMetadata(connection.id, profile.field_type.id, profile.url));
        }
      }
  ...
  return (
    <Form
      ...
      navigationTitle={`Refresh Metadata: ${connection.name}`}
      actions={
        <ActionPanel>
