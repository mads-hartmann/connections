...
export function ConnectionCreateForm({ revalidate }: CreateConnectionFormProps) {
  ...
  async function handleSubmit(values: { url: string }) {
    ...
    try {
      ...
      push(<ConnectionPreviewForm metadata={metadata} sourceUrl={url} revalidate={revalidate} />);
    } catch (error) {
      showToast({
        style: Toast.Style.Failure,
  ...
  return (
    ...
    >
      ...
      <Form.Description text="Paste a URL to automatically extract the connection's name and discover their RSS feeds." />
    </Form>
  );
}
...
async function createConnection(
  ...
  if (profiles && profiles.length > 0) {
    for (const profile of profiles) {
      await fetch(`${getServerUrl()}/connections/${connection.id}/metadata`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
function ConnectionPreviewForm({ metadata, sourceUrl, revalidate }: ConnectionPreviewFormProps) {
  ...
  async function handleSubmit(values: Record<string, unknown>) {
    ...
    if (!name.trim()) {
      showToast({
        style: Toast.Style.Failure,
        title: "Name required",
        message: "Please enter a name for the connection",
      });
      return;
    }
